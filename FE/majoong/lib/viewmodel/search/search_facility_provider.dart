...
final searchCenterPositionProvider =
StateProvider.autoDispose<GetFacilityRequestDto>((ref) {
  return GetFacilityRequestDto(centerLng: 0, centerLat: 0, radius: 0);
});

final searchCameraMovedProvider = StateProvider.autoDispose<bool>((ref) {
  return false;
});

final searchFacilityProvider =
StateNotifierProvider.autoDispose<SearchFacilityNotifier, BaseResponseState>(
        (ref) {
      final mapService = ref.watch(mapApiServiceProvider);
      final markerInfo =
      ref.watch(searchMarkerProvider.notifier);
      final chipInfo = ref.watch(searchChipProvider.notifier as AlwaysAliveProviderListenable);
      final reviewDialogInfo = ref.watch(reviewDialogProvider.notifier);
      final centerPositionInfo = ref.watch(searchCenterPositionProvider.notifier as AlwaysAliveProviderListenable);
