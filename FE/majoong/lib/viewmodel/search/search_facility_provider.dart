...
final searchCenterPositionProvider = StateProvider<GetFacilityRequestDto>((ref) {
  return GetFacilityRequestDto(centerLng: 0, centerLat: 0, radius: 0);
});

final searchCameraMovedProvider = StateProvider<bool>((ref) {
  return false;
});

final searchFacilityProvider =
StateNotifierProvider<SearchFacilityNotifier, BaseResponseState>((ref) {
  ...
  final centerPositionInfo = ref.watch(searchCenterPositionProvider.notifier);
  final polyLineInfo =
  ref.watch(searchPolyLineProvider.notifier as AlwaysAliveProviderListenable);
  final polygonInfo =
  ref.watch(searchPolygonProvider.notifier as AlwaysAliveProviderListenable);

  final dio = ref.watch(dioProvider);
  final facilityNotifier = SearchFacilityNotifier(
    dio,
    service: mapService,
...
class SearchFacilityNotifier extends StateNotifier<BaseResponseState> {
  final MapApiService service;
  final SearchMarkerNotifier markerNotifier;
  final SearchChipNotifier chipNotifier;
  final SearchPolyLineNotifier polyLineNotifier;
  final SearchPolygonNotifier polygonNotifier;
  final StateNotifier centerPositionNotifier;
  final ReviewDialogNotifier reviewDialogNotifier;
  final Dio dio;

  SearchFacilityNotifier(this.dio,
